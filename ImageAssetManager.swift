...
class ImageAssetManager: NSObject {
    ...
    func loadFileURLSFromFolderInBundle(parentFolder: String, completion: (_ result:[[Data]]?, _ status: FoundImagesInBundle) -> ()){
        
        let imageDirectoryUrls = returnImagePath(parentFolder: parentFolder, subDirectory: nil)
        ...
        completion(assetsArray, .success)
        
        }
    
    func returnImagePath(parentFolder: String, subDirectory: String?) -> [URL] {
